...
function Adjust-Color ($col, $amt) {
    ...
    $num = [System.Convert]::ToInt32($col, 16)
    
    $r = (($num -shr 16) -band 0xFF) + $amt
    if ($r -gt 255) { $r = 255 } elseif ($r -lt 0) { $r = 0 }
    
    $g = (($num -shr 8) -band 0xFF) + $amt
    if ($g -gt 255) { $g = 255 } elseif ($g -lt 0) { $g = 0 }
    
    $b = ($num -band 0xFF) + $amt
    if ($b -gt 255) { $b = 255 } elseif ($b -lt 0) { $b = 0 }
    
    $res = ($r -shl 16) -bor ($g -shl 8) -bor $b
    $hex = $res.ToString("X6")
    if ($usePound) { return "#$hex" }
    return $hex
}
...
[System.IO.File]::WriteAllText((Join-Path $outputDir "logo.svg"), $logoSvg)

function Get-ShoeSVG ($type, $colors) {
    ...
    $darkPrimary = Adjust-Color $primary -30
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  ...
  <g filter="url(#shadow)" transform="translate(10, 15)">
    ...
    <path d="M 100,105 C 90,130 95,190 120,200 C 150,200 240,200 300,200 C 320,200 330,190 325,175 C 320,160 305,145 280,140 C 250,135 220,120 185,115 C 165,110 145,102 125,100 C 112,100 105,102 100,105 Z" fill="url(#primaryGrad)" />
    ...
    <path d="M 105,195 C 120,195 280,195 305,195 Q 318,195 322,190 L 325,197 Q 322,207 305,207 L 108,207 Q 100,202 105,195 Z" fill="$sole" />
...
"@
    ...
    if ($type -eq 'running') {
        $soleAccent = Adjust-Color $sole 20
        return @"
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  <defs>
    <linearGradient id="bgGrad" x1="0%" y1="0%" x2="100%" y2="100%">
    ...
    <linearGradient id="soleGrad" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <stop offset="100%" stop-color="$soleAccent" />
    </linearGradient>
    <filter id="shadow" x="-10%" y="-10%" width="120%" height="130%">
  ...
  <g filter="url(#shadow)" transform="translate(10, 15)">
    ...
    <path d="M 90,95 C 80,120 82,185 110,195 C 140,195 250,190 310,185 C 335,183 345,172 340,158 C 330,140 310,125 280,120 C 240,115 200,90 170,88 C 145,86 125,88 110,90 C 98,92 92,92 90,95 Z" fill="url(#primaryGrad)" />
    ...
    <path d="M 98,192 C 120,192 270,185 315,182 Q 335,180 342,175 L 340,185 Q 330,195 315,200 L 260,205 Q 200,212 140,212 L 100,206 Q 90,202 98,192 Z" fill="url(#soleGrad)" />
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  ...
  <g filter="url(#shadow)" transform="translate(10, 10)">
    ...
    <path d="M 100,115 C 85,130 90,195 115,200 C 145,200 250,200 305,200 C 325,200 338,190 335,170 C 330,150 310,130 280,120 C 255,110 215,95 175,90 C 150,86 132,100 120,112 Z" fill="url(#primaryGrad)" />
    ...
    <rect x="125" y="202" width="40" height="10" fill="#ffffff" opacity="0.1" />
    <path d="M 160,110 L 185,122 M 172,98 L 198,110 M 185,88 L 210,100" stroke="#ffffff" stroke-width="3" stroke-linecap="round" />
    <path d="M 105,195 C 120,195 280,196 308,196 Q 322,196 328,190 L 332,198 Q 325,210 308,212 L 110,212 Q 98,205 105,195 Z" fill="$sole" />
...
function Get-AccessorySVG ($item, $colors) {
    $primary = $colors.primary
    $secondary = $colors.secondary
    $accent = $colors.accent
    $bgStart = $colors.bgStart
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  ...
  <g filter="url(#shadow)" transform="translate(40, 20)">
    ...
    <g transform="translate(30, 20)">
      <path d="M 140,60 L 180,60 C 185,120 185,150 180,165 L 235,210 C 245,218 240,230 225,230 L 205,230 L 145,180 C 138,160 138,120 140,60 Z" fill="#ffffff" />
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  ...
  <g filter="url(#shadow)" transform="translate(50, 40)">
    <path d="M 50,150 C 50,50 180,50 180,110 C 180,170 80,180 80,130 C 80,90 150,90 150,120 C 150,150 110,150 110,135" fill="none" stroke="$accent" stroke-width="8" stroke-linecap="round" stroke-dasharray="8 4"/>
    <path d="M 120,160 C 120,60 250,60 250,120 C 250,180 150,190 150,140 C 150,100 220,100 220,130 C 220,160 180,160 180,145" fill="none" stroke="$primary" stroke-width="8" stroke-linecap="round"/>
...
function Get-OtherSVG ($item, $colors) {
    $primary = $colors.primary
    $secondary = $colors.secondary
    $accent = $colors.accent
    $bgStart = $colors.bgStart
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  ...
  <g filter="url(#shadow)" transform="translate(20, 20)">
    ...
    <path d="M 80,185 C 70,140 90,110 120,105 C 150,100 240,95 285,115 C 305,125 310,145 300,165 C 290,180 260,195 220,195 C 170,195 100,200 80,185 Z" fill="$primary"/>
    ...
    <path d="M 180,105 C 185,85 240,85 255,115 C 260,125 260,165 245,182 C 235,160 230,125 180,105 Z" fill="$secondary"/>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 300" width="100%" height="100%">
  ...
  <g filter="url(#shadow)" transform="translate(20, 20)">
    ...
    <path d="M 90,140 C 80,100 110,80 130,80 L 160,82 C 200,95 240,110 280,135 C 305,150 310,180 290,195 C 270,205 240,205 180,205 C 120,205 100,190 90,140 Z" fill="#ffffff" fill-opacity="0.4" stroke="#ffffff" stroke-width="3"/>
...
# Output each product SVG
foreach ($p in $products) {
    $filePath = Join-Path $outputDir $p.file
    $svg = ""
    if ($p.type -eq 'casual' -or $p.type -eq 'running' -or $p.type -eq 'basketball') {
        $svg = Get-ShoeSVG -type $p.type -colors $p.colors
    } elseif ($p.type -eq 'socks' -or $p.type -eq 'cleaner' -or $p.type -eq 'laces') {
        $svg = Get-AccessorySVG -item $p.type -colors $p.colors
    } elseif ($p.type -eq 'sandal' -or $p.type -eq 'rainshield') {
        $svg = Get-OtherSVG -item $p.type -colors $p.colors
    }
